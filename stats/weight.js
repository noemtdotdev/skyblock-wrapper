...
function formatLilyDungeons(dungeons) {
    // Safely access catacombs properties or use fallback values
    const catacombs_experience = dungeons?.catacombs?.skill?.totalXp || 0;
    const catacombsFloors = dungeons?.catacombs?.floors || {};
    const masterModeFloors = dungeons?.catacombs?.master_mode_floors || {};

    const catacombs = Object.keys(catacombsFloors).reduce((acc, floor) => {
        const formattedFloor = floor === "entrance" ? 0 : floor.split('_')[1];
        acc[formattedFloor] = catacombsFloors[floor].completions;
        return acc;
    }, {});

    const master_mode = Object.keys(masterModeFloors).reduce((acc, floor) => {
        acc[floor.split('_')[1]] = masterModeFloors[floor].completions;
        return acc;
    }, {});
