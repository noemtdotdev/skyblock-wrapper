...
function safelyGetStat(profile, statKey) {
  return profile.stats && profile.stats[statKey] ? profile.stats[statKey] : 0;
}

module.exports = function getMilestones(profile) {
  const dolphin = getCurrentPet(
    "dolphin",
    safelyGetStat(profile, 'pet_milestone_sea_creatures_killed')
  );
  const rock = getCurrentPet(
    "rock",
    safelyGetStat(profile, 'pet_milestone_ores_mined')
  );
