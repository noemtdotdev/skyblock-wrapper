...
module.exports = (player, profile) => {
  const jacob = {
    talked: profile.jacobs_contest?.talked || false,
  };
  const trapper_quest = {
    last_task_time: "None",
    pelt_count: 0,
  ...
  if (jacob.talked) {
    ...
    jacob.perks = {
      double_drops: profile.jacobs_contest.perks?.double_drops || 0,
      farming_level_cap: profile.jacobs_contest.perks?.farming_level_cap || 0,
    };
    jacob.unique_golds = profile.jacobs_contest.unique_brackets?.gold?.length || 0;
    jacob.crops = {};
    ...
    for (const crop in constants.jacob_crops) {
      ...
      Object.assign(jacob.crops[crop], {
        participated: false,
        unique_gold: profile.jacobs_contest.unique_brackets?.gold?.includes(crop) || false,
        contests: 0,
        personal_best: 0,
        badges: {
          gold: 0,
    ...
    for (const contest_id in profile.jacobs_contest.contests) {
      const data = profile.jacobs_contest.contests[contest_id];
