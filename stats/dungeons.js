...
module.exports = (player, profile, secrets) => {
  try {
    if (!profile?.dungeons) {
      console.error("dungeons data error");
      return null;
    }
    ...
    const floors = {};
    const available_floors = Object.keys(catacombs?.times_played || []);

    for (const floor of available_floors) {
      let floor_name = floor == 0 ? "entrance" : `floor_${floor}`;
      floors[floor_name] = {
      ...
      for (const key of Object.keys(catacombs)) {
        if (key.startsWith("most_damage") || key.startsWith("most_healing")) {
          floors[floor_name][key] = catacombs[key]?.[floor] ?? (key.endsWith("_healing") ? [] : 0);
        }
      }
    }
    ...
    for (let i = 1; i <= (dungeons?.dungeon_types?.master_catacombs?.highest_tier_completed ?? 0); i++) {
      ...
      for (const key of Object.keys(master_catacombs)) {
        if (key.startsWith("most_damage") || key.startsWith("most_healing")) {
          master_mode_floors[`floor_${i}`][key] = master_catacombs[key]?.[i] ?? (key.endsWith("_healing") ? [] : 0);
        }
      }
    }
