...
module.exports = wrap(async function (req, res) {
  ...
  let museumRes;
  let gardenRes;
  let profileData; // Declare profileData in broader scope
  ...
  try {
    if (!playerRes) {
      ...
      try {
        // Cache only the serializable data parts
        const cacheableData = {
    ...
    // Try to get cached museum and garden data
    try {
    ...
    if (!museumRes) {
      ...
      if (museumResponse.status === 404) {
        ...
        try {
          ...
          await client.set(cacheKeyMuseum, JSON.stringify(cacheableData), { EX: 120 });
          console.log(
    ...
    if (!gardenRes) {
      ...
      if (gardenResponse.status === 404) {
        ...
        try {
          ...
          await client.set(cacheKeyGarden, JSON.stringify(cacheableData), { EX: 120 });
          console.log(
  ...
  try {
    // Additional validation before processing profile names
    if (profileData && profileData.profiles && Array.isArray(profileData.profiles)) {
      for (const profile of profileData.profiles) {
